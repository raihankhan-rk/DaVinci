        ...
        with st.spinner("Minting NFT..."):
            if st.session_state.name and st.session_state.desc and st.session_state.mint_to_address:
                ...
                st.info(f"""
                         ...
                         """)
                st.write("Check out your NFT on OpenSea [https://opensea.io/account/private](https://opensea.io/account/private)")
            else:
...
if st.button('Generate NFT'):
    ...
    with st.form(key='minting_form', clear_on_submit=False):
        ...
        desc = st.text_area(label='', label_visibility='hidden', placeholder='Write a short description about your NFT', key='desc')
        mint_to_address = st.text_input(label='', label_visibility='hidden', placeholder='Minting Wallet Address', key='mint_to_address')
